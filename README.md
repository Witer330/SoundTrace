...
- [docs/开发记录.md](docs/开发记录.md) —— 已完成里程碑的技术实现与验证结果
...
1. 启动后进入「设置」→ 下载转写模型（约 310MB，本地离线推理）
2. 回到「录音库」，拖入手机录音（文件或整个文件夹）
3. 打开任意录音 → 「开始转写」；完成后点转写稿任意文字即跳到对应音频位置
4. 需要会议纪要时，在设置里填好 LLM（Base URL / API Key / 模型），详情页点「生成纪要」
